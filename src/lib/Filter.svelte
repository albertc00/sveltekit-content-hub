<script>
	...
	let selectedPropID;
	$: selectedPropText = properties.find(({ id }) => selectedPropID === id)?.text;
	let selectedCompare = null;
	let value = [];

	$: disableInput = value.length === 3 ? true : false;
	...
	const withValue = compares.filter(({ withValue }) => withValue).map(({ compare }) => compare);

	$: if (selectedCompare && !withValue.includes(selectedCompare) && value.length) {
		value = [];
	}
	...
	function handleSubmit() {
		onApply(filters.map(({ value, ...rest }) => (value?.length ? { value, ...rest } : rest)));
	}
	...
	$: _properties = properties.filter(({ id }) => !filters.some(({ key }) => key === id));
	...
	let filterRefs = [];
	$: disabled = filterRefs.map((filterRef) => filterRef && !hasEllipses(filterRef));
</script>
...
<form class="filter-wrapper" on:submit|preventDefault={handleSubmit}>
	<div class="inner-wrapper">
		...
		{:else if step === 1}
				...
				{#each compares as { text, compare, withValue } (compare)}
					<div class="compare">
						...
						{#if withValue && selectedCompare === compare}
							<div>
								<MultiTermInput bind:keywords={value} {disableInput} />
							</div>
						{/if}
					</div>
				{/each}
			...
			<Button
				disabled={!selectedCompare || (withValue.includes(selectedCompare) && !value.length)}
				color="blue-400"
				on:click={() => handleAdd({ key: selectedPropID, compare: selectedCompare, value })}
		...
		{:else if step === 2}
			<div class="filters">
				{#each _filters as { key, text, compareText, value }, i (key)}
					<div
						class="filter"
						on:click|preventDefault={() => handleEdit(key)}
						on:keypress={() => {}}
					>
						<Tooltip
							title={`${text} ${compareText}${value?.length ? ` "${joinOr(value)}"` : ''}`}
							disabled={disabled[i]}
						>
			...
			<button
				...
				on:click|preventDefault={handleAnd}>{filters.length ? 'and' : 'add'}</button
			>
			<div class="button-row">
				<Button type="submit" color="blue-400">Apply filter</Button>
				<Button filled={false} color="blue-400" preventDefault on:click={handleCancel}
					>Cancel</Button
				>
